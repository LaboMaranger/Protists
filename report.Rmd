...
ReporteRsWrap<- function(FUN,filename="Figure1"){
  ...
  doc <- addPlot(doc, FUN, vector.graphic = TRUE )
  writeDoc(doc, file = paste0("figures/",filename,".pptx"))
}
...
# proportion by taxa
transect_seq.prop=prop.table(as.matrix(transect_seq_agg),margin=1)

# logit transformation (for percentages)
transect_seq.logit=car::logit(transect_seq.prop)
# add the minimum (permanova = no negative values)
transect_seq.logit=transect_seq.logit+abs(min(transect_seq.logit))


print(fit <-adonis(transect_seq.logit~Year*stationGr,method="bray",permutations=999,data = 
samdf_agg[rownames(transect_seq.logit),]),permutations = 9999)

print(fit <-adonis(transect_seq.logit~DSP.4+chla.Peak+Year*stationGr,method="bray",permutations=999,data = 
samdf_agg[rownames(transect_seq.logit),]),permutations = 9999)
...
# permonava with the proportion of each trait, logit transform
print(fit <-adonis(com_prop.logit
~Year*stationGr,method="bray",permutations=999,data = 
samdf_agg[rownames(transect_seq.logit),]),permutations = 9999)

print(fit <-adonis(com_prop.logit
~DSP.4+chla.Peak+Year*stationGr,method="bray",permutations=9999,data = samdf_agg[rownames(com_prop),]))
...
print(fit <-adonis(com_prop.logit+abs(min(com_prop.logit))
~DSP.4+chla.Peak+Year*stationGr,method="bray",permutations=9999,data = samdf_agg[rownames(com_prop),]))
...
transect_fs_C1=prop.table(mergeByCat(transect_seq_agg[,sizeClasses%in%"(0.2,2]"],as.factor(fsVector_seq)),margin=1)
transect_fs_C2=prop.table(mergeByCat(transect_seq_agg[,sizeClasses%in%"(2,5]"],as.factor(fsVector_seq)),margin=1)
transect_fs_C3=prop.table(mergeByCat(transect_seq_agg[,sizeClasses%in%"(5,10]"],as.factor(fsVector_seq)),margin=1)
transect_fs_C4=prop.table(mergeByCat(transect_seq_agg[,sizeClasses%in%"(10,20]"],as.factor(fsVector_seq)),margin=1)
transect_fs_C5=prop.table(mergeByCat(transect_seq_agg[,sizeClasses%in%"(20,Inf]"],as.factor(fsVector_seq)),margin=1)
